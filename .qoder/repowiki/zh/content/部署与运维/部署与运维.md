...
- [.prettierrc](file://apps/AgentPit/.prettierrc)
- [eslint.config.js](file://apps/AgentPit/eslint.config.js)
- [vitest.config.ts](file://apps/AgentPit/vitest.config.ts)
- [tsconfig.json](file://apps/AgentPit/tsconfig.json)
- [tsconfig.app.json](file://apps/AgentPit/tsconfig.app.json)
- [dev-environment.yml](file://tools/flexloop/tests/testing/dev-environment.yml)
</cite>

## 更新摘要
**所做更改**
- 新增 GitHub Actions CI/CD 流水线配置章节，详细介绍自动化测试和部署流程
- 更新 CI/CD 流水线配置，包括 Prettier 格式检查、ESLint 代码检查、TypeScript 类型检查、单元测试和覆盖率报告
- 增强前端质量保证流程说明，涵盖代码格式化、静态分析和测试覆盖
- 更新开发环境搭建指南，包含 Husky 预提交钩子和 lint-staged 配置
...
- 应用层（前端）：AgentPit（Vue3 + Vite），通过包管理脚本与构建配置进行本地开发与打包，集成完整的 CI/CD 流水线。
...
graph TB
subgraph "应用层"
AP["AgentPit<br/>Vue3 + Vite<br/>CI/CD 流水线"]
end
subgraph "工具层"
...
- 前端应用（AgentPit）
  ...
  - 集成完整的质量保证流程，包括 Prettier 格式检查、ESLint 代码检查、TypeScript 类型检查和 Vitest 单元测试。
- 后端工具（DeepResearch）
...
Nginx --> FE["前端应用(AgentPit)<br/>Vite 构建产物(dist)<br/>CI/CD 自动化"]
...
- 质量保证
  - Prettier 格式检查：统一代码风格，支持自动修复和检查模式。
  - ESLint 代码检查：基于 TypeScript ESLint 和 Vue 插件的推荐配置。
  - TypeScript 类型检查：使用 vue-tsc 进行类型验证。
  - Vitest 单元测试：支持覆盖率报告和多种报告格式。

```mermaid
flowchart TD
Start(["开始"]) --> QualityCheck["执行质量检查<br/>Prettier + ESLint + TypeScript"]
QualityCheck --> TypeCheck["类型检查<br/>vue-tsc"]
TypeCheck --> UnitTest["单元测试<br/>Vitest + 覆盖率"]
UnitTest --> Build["构建项目<br/>Vite + Vue"]
Build --> Dist["生成 dist 目录"]
Dist --> Ignore["忽略 dist 与 node_modules"]
Ignore --> End(["结束"])
```
...
### GitHub Actions CI/CD 流水线概述
本项目采用 GitHub Actions 实现完整的持续集成与持续部署流水线，确保代码质量与部署可靠性。流水线在每次推送和拉取请求时自动触发，执行一系列质量检查和测试步骤。

```mermaid
flowchart TD
Start(["Git 推送/PR 触发"]) --> Checkout["代码检出<br/>actions/checkout@v4"]
Checkout --> SetupNode["设置 Node.js 环境<br/>actions/setup-node@v4"]
SetupNode --> InstallDeps["安装依赖<br/>npm ci"]
InstallDeps --> PrettierCheck["Prettier 格式检查<br/>npm run format:check"]
PrettierCheck --> ESLintCheck["ESLint 代码检查<br/>npm run lint:check"]
ESLintCheck --> TypeCheck["TypeScript 类型检查<br/>npx vue-tsc -b"]
TypeCheck --> UnitTests["单元测试<br/>npm run test:run"]
UnitTests --> Coverage["覆盖率报告<br/>Codecov"]
Coverage --> Build["生产环境构建<br/>npm run build"]
Build --> Artifacts["上传构建产物<br/>actions/upload-artifact@v4"]
Artifacts --> End(["流水线完成"])
```
...
- 设置 Node.js 20.x 环境，启用 npm 缓存以提高构建速度

#### 2. 代码质量检查
- **Prettier 格式检查**：`npm run format:check` 确保代码风格一致性
- **ESLint 代码检查**：`npm run lint:check` 执行静态代码分析
- **TypeScript 类型检查**：`npx vue-tsc -b` 验证类型安全性

#### 3. 测试与覆盖率
- **单元测试**：`npm run test:run` 执行所有测试用例
- **覆盖率报告**：使用 `@vitest/coverage-v8` 生成 lcov 报告，上传至 Codecov
- **测试配置**：Vitest 配置支持多种报告格式（text、json、html、lcov）

#### 4. 构建与部署
- **生产构建**：`npm run build` 生成优化后的生产版本
- **构建产物**：上传 dist 目录作为构建工件，便于后续部署
...
#### Husky 预提交钩子
项目集成了 Husky 预提交钩子，确保在提交代码前自动执行质量检查：

```mermaid
flowchart LR
Commit["git commit"] --> PreCommit["husky pre-commit 钩子"]
PreCommit --> LintStaged["lint-staged 执行"]
LintStaged --> ESLintFix["ESLint 自动修复"]
LintStaged --> PrettierWrite["Prettier 自动格式化"]
ESLintFix --> CommitOK["提交成功"]
PrettierWrite --> CommitOK
```

**图表来源**
- [package.json:15-16](file://apps/AgentPit/package.json#L15-L16)
- [package.json:64-72](file://apps/AgentPit/package.json#L64-L72)

#### lint-staged 配置
- JavaScript/TypeScript/Vue 文件：先执行 ESLint 修复，再进行 Prettier 格式化
- JSON/Markdown/CSS 文件：仅执行 Prettier 格式化
- 忽略规则：排除 dist、node_modules、配置文件和测试目录
...
- Tab 宽度：`"tabWidth": 2`
- 禁用 tab：`"useTabs": false`
- 行尾逗号：`"trailingComma": "none"`
- 行宽：`"printWidth": 100`
- 分号：`"semi": true`

#### ESLint 配置
- 基于 TypeScript ESLint 和 Vue 插件的推荐配置
- 忽略规则：排除 dist、node_modules、配置文件和测试目录
- 全局变量：定义浏览器和 Node.js 环境的全局对象
- 规则调整：根据生产环境需求调整警告级别
...
**章节来源**
- [.prettierrc:1-1](file://apps/AgentPit/.prettierrc#L1-L1)
- [eslint.config.js:1-162](file://apps/AgentPit/eslint.config.js#L1-L162)
- [tsconfig.json:1-8](file://apps/AgentPit/tsconfig.json#L1-L8)
- [tsconfig.app.json:1-1](file://apps/AgentPit/tsconfig.app.json#L1-L1)

## 结论
本仓库提供了清晰的前端与后端部署边界：前端通过 Vite 快速构建并由反向代理对外提供服务，后端通过 Python CLI 与配置文件实现独立部署与扩展。结合统一的工程脚本、质量工具和完整的 GitHub Actions CI/CD 流水线，可在 CI/CD 中实现自动化测试、构建与发布。

**更新** 新增的 CI/CD 流水线显著提升了代码质量和部署效率，包括 Prettier 格式检查、ESLint 代码检查、TypeScript 类型检查、单元测试和覆盖率报告等完整质量保证流程。

建议在生产环境中配套完善的监控、日志与告警体系，并制定安全与备份恢复策略以保障系统稳定运行。
...
- 前端
  ...
  - 可选择使用 GitHub Actions 生成的构建工件进行部署。
- 后端
...
- 质量保证
  - Prettier 格式检查：统一代码风格
  - ESLint 代码检查：静态代码分析
  - TypeScript 类型检查：类型安全性验证
  - Vitest 单元测试：功能测试与覆盖率报告
