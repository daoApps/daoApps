...
<template>
  <div class="space-y-5">
    ...
    <div
      ...
      class="flex items-center justify-between px-4 py-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800"
    >
      <span class="text-sm text-blue-700 dark:text-blue-300">已选择 {{ selectedIds.size }} 项</span>
      <div class="flex gap-2">
        <button
          class="px-3 py-1.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-xs font-medium hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
          @click="confirmDelete()"
        >
          批量删除
        </button>
        <button
          class="px-3 py-1.5 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 text-xs transition-colors"
      ...
      >
        ...
        <div class="p-5 pt-8">
          <div class="flex items-start gap-3 mb-3">
            <div
              class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/40 dark:to-purple-900/40 flex items-center justify-center text-2xl shrink-0 shadow-sm"
            >
              {{ agent.avatar }}
            </div>
          ...
          <div class="flex flex-wrap gap-1 mb-3">
            <span
              ...
              >{{ tag }}</span
            >
            <span v-if="agent.tags.length > 3" class="text-[10px] text-gray-400"
              >+{{ agent.tags.length - 3 }}</span
            >
          </div>
        ...
        <div
          class="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-white/95 dark:from-gray-800/95 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-200 flex justify-end gap-1.5"
        >
          <button
            class="px-3 py-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg text-xs font-medium hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
            @click="handleEdit(agent)"
          >
            编辑
          </button>
          <button
            class="px-3 py-1.5 bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-lg text-xs font-medium hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
          >
            复制
          </button>
          <button
            v-if="agent.status === 'draft'"
            class="px-3 py-1.5 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg text-xs font-medium hover:bg-green-100 dark:hover:bg-green-900/50 transition-colors"
          >
            发布
          </button>
          <button
            v-if="agent.status === 'published'"
            class="px-3 py-1.5 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-lg text-xs font-medium hover:bg-amber-100 dark:hover:bg-amber-900/50 transition-colors"
          >
            停用
          </button>
          <button
            class="px-3 py-1.5 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg text-xs font-medium hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors"
    ...
    <div v-else-if="filteredAgents.length === 0" class="text-center py-16">
      ...
      <button
        class="px-6 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors inline-flex items-center gap-1.5"
        @click="$emit('create')"
      >
        <span>+</span> 创建智能体
      </button>
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          ...
          <div
            class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-scale-in"
          >
            <div class="text-center">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">
                {{
                  deleteTargetId
                    ? '确定要删除此智能体吗？'
                    : `确定要删除选中的 ${selectedIds.size} 个智能体吗？`
                }}
            ...
            <div class="flex gap-3 mt-6">
              <button
                class="flex-1 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
