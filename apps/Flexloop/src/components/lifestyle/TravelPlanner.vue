...
function deleteActivity(dayIdx: number, activityId: string) {
  if (currentPlan.value) {
    currentPlan.value.days[dayIdx].activities = currentPlan.value.days[dayIdx].activities.filter(
      (a) => a.id !== activityId
    );
    updateTotalCost();
  }
...
const totalActivities = computed(
  () => currentPlan.value?.days.reduce((sum, d) => sum + d.activities.length, 0) || 0
);
  ...
  <div class="travel-planner space-y-6">
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
      ...
      <div class="flex gap-2 text-sm text-gray-600 dark:text-gray-400">
        <span
          class="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 rounded-lg font-medium text-indigo-600 dark:text-indigo-400"
          >{{ totalDays }} 天</span
        >
        <span
          class="px-3 py-1.5 bg-green-50 dark:bg-green-900/30 rounded-lg font-medium text-green-600 dark:text-green-400"
    ...
    <div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <div
          v-for="dest in filteredDestinations.slice(0, 8)"
          :key="dest.id"
        ...
        >
          <div class="aspect-[4/3] overflow-hidden">
            ...
            >
              <svg
                class="w-5 h-5"
                :class="isFavorite(dest.id) ? 'text-red-500 fill-red-500' : 'text-white'"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div class="p-3">
            ...
            <div class="flex flex-wrap gap-1">
              <span
                ...
                :key="tag"
                class="px-2 py-0.5 text-xs rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400"
                >{{ tag }}</span
              >
            </div>
          </div>
    ...
    <div v-if="currentPlan">
      ...
      <div class="relative pl-8">
        <div
          class="absolute left-3 top-0 bottom-0 w-0.5 bg-gradient-to-b from-indigo-400 via-purple-400 to-pink-400"
        ></div>

        <div v-for="(day, dayIdx) in currentPlan.days" :key="day.day" class="mb-8 relative">
          <div
            class="absolute -left-5 top-0 w-4 h-4 rounded-full bg-indigo-500 ring-4 ring-white dark:ring-gray-800"
          ></div>
          <div
          ...
          >
            ...
            <div class="space-y-2">
              <TransitionGroup name="activity-list">
                ...
                >
                  <span
                    class="mt-0.5 px-2 py-0.5 text-xs font-mono font-medium rounded bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 flex-shrink-0"
                    >{{ activity.time }}</span
                  >
                  <div class="flex-1 min-w-0">
                    <h5 class="font-medium text-gray-900 dark:text-white">{{ activity.title }}</h5>
                    <p class="text-xs text-gray-500 mt-0.5">
                      📍 {{ activity.location }}{{ activity.notes ? ` · ${activity.notes}` : '' }}
                    </p>
                  </div>
                  <span
                    v-if="activity.cost"
                  ...
                  <button
                    ...
                    @click="deleteActivity(dayIdx, activity.id)"
                  >
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div
          v-if="showDestinationDetail && selectedDestination"
          class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm"
          @click.self="showDestinationDetail = false"
        >
          <div
          ...
          >
            ...
            <div class="p-5">
              ...
              <div class="flex flex-wrap gap-2 mb-4">
                <span
                  v-for="tag in selectedDestination.tags"
                  :key="tag"
                  class="px-2.5 py-1 text-xs rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400"
                  >{{ tag }}</span
                >
              </div>
              <p class="text-lg font-bold text-indigo-600 dark:text-indigo-400">
                预计费用：约 ¥{{ selectedDestination.estimatedCost }}/天
              </p>
            </div>
            <div class="p-4 border-t border-gray-200 dark:border-gray-700 flex gap-3">
              <button
                :class="[
                  ...
                  isFavorite(selectedDestination.id)
              ...
              <button
                ...
                @click="showDestinationDetail = false"
              >
                开始规划
              </button>
            </div>
