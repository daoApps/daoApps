...
const recentActivities = computed<RecentActivity[]>(() => {
  ...
  if (todayTip.value) {
    activities.push({
      id: 'tip-1',
      type: 'tip',
      title: `💡 ${todayTip.value.content.substring(0, 30)}...`,
      time: '今日',
      icon: '💡'
    });
  }
...
<template>
  <div class="lifestyle-dashboard space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div class="flex items-center justify-between mb-3">
          ...
          <span class="text-xs font-medium bg-white/20 px-2 py-1 rounded-full"
            >{{ todayMeetings.length }} 个会议</span
          >
        </div>
        <h3 class="text-2xl font-bold">{{ todayMeetings.length }}</h3>
        <p class="text-sm text-blue-100 mt-1">今日会议数</p>
        <p
          v-if="todayMeetings[0]"
          class="text-xs text-blue-200 mt-2 truncate opacity-0 group-hover:opacity-100 transition-opacity"
        >
      ...
      <!-- 待办事项 -->
      <div
        class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition-shadow cursor-pointer group"
      >
        <div class="flex items-center justify-between mb-3">
          <span class="text-3xl">⏰</span>
          <span class="text-xs font-medium bg-white/20 px-2 py-1 rounded-full"
            >{{ pendingTodos.length }} 待处理</span
          >
        </div>
        ...
        <p
          v-if="pendingTodos[0]"
          class="text-xs text-green-200 mt-2 truncate opacity-0 group-hover:opacity-100 transition-opacity"
        >
      ...
      <!-- 游戏成就 -->
      <div
        class="bg-gradient-to-br from-purple-500 to-violet-600 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition-shadow cursor-pointer group"
      >
        <div class="flex items-center justify-between mb-3">
          <span class="text-3xl">🎮</span>
        ...
        <p
          class="text-xs text-purple-200 mt-2 opacity-0 group-hover:opacity-100 transition-opacity"
        >
          贪吃蛇 · 俄罗斯方块 · 2048
        </p>
      </div>
      ...
      <!-- 生活小贴士 -->
      <div
        class="bg-gradient-to-br from-amber-400 to-orange-500 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition-shadow cursor-pointer"
      >
        <div class="flex items-center justify-between mb-3">
          <span class="text-3xl">{{ todayTip?.icon || '💡' }}</span>
          <span class="text-xs font-medium bg-white/20 px-2 py-1 rounded-full">每日一贴</span>
        </div>
        <h3 class="text-base font-bold line-clamp-2 leading-tight">
          {{ todayTip?.content || '' }}
        </h3>
      ...
      <!-- 本周统计 -->
      <div
        class="col-span-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm"
      >
        <h3 class="font-bold text-gray-900 dark:text-white mb-4">📊 本周活动统计</h3>
        <div class="flex items-end gap-2 h-[160px]">
          <div
            v-for="(day, idx) in weeklyActivities"
            :key="day.day"
          ...
          >
            <div class="w-full flex gap-0.5 items-end h-[130px]">
              <div
                ...
                :style="{
                  height: getBarHeight(dataset.data[idx]),
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      <button
        class="flex items-center gap-3 p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors group"
      >
        <span class="text-2xl group-hover:scale-110 transition-transform">📅</span>
        <span class="font-medium text-blue-700 dark:text-blue-300 text-sm">日历管理</span>
      </button>
      <button
        class="flex items-center gap-3 p-4 rounded-xl bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors group"
      >
        <span class="text-2xl group-hover:scale-110 transition-transform">✈️</span>
        <span class="font-medium text-green-700 dark:text-green-300 text-sm">旅游规划</span>
      </button>
      <button
        class="flex items-center gap-3 p-4 rounded-xl bg-purple-50 dark:bg-purple-900/20 hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors group"
      >
        <span class="text-2xl group-hover:scale-110 transition-transform">🎮</span>
        <span class="font-medium text-purple-700 dark:text-purple-300 text-sm">游戏中心</span>
      </button>
      <button
        class="flex items-center gap-3 p-4 rounded-xl bg-orange-50 dark:bg-orange-900/20 hover:bg-orange-100 dark:hover:bg-orange-900/30 transition-colors group"
      >
        <span class="text-2xl group-hover:scale-110 transition-transform">⚙️</span>
    ...
    >
      ...
      <div class="relative pl-6">
        <div
          class="absolute left-2 top-2 bottom-2 w-0.5 bg-gradient-to-b from-indigo-300 to-purple-300"
        ></div>
        <div class="space-y-3">
          <TransitionGroup name="activity-list">
            ...
            >
              <div
                class="absolute -left-6 top-1 w-3 h-3 rounded-full bg-white dark:bg-gray-800 ring-2 ring-indigo-400"
              ></div>
              <div
                class="flex-1 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50 group-hover:bg-gray-100 dark:group-hover:bg-gray-700 transition-colors"
              >
