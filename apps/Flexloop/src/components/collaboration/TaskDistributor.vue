...
const onDrop = (event: DragEvent, newStatus: string) => {
  ...
  if (taskIndex > -1) {
    ...
    updatedTasks[taskIndex] = {
      ...
      ...(newStatus === 'in_progress' && !updatedTasks[taskIndex].startTime
        ? { startTime: Date.now() }
        : {})
    };
...
const stats = computed(() => ({
  ...
  avgProgress: Math.round(
    tasks.value.reduce((acc, t) => acc + t.progress, 0) / tasks.value.length || 0
  )
}));
</script>
...
<template>
  ...
  >
    ...
    <div class="p-4 border-b border-gray-200 dark:border-gray-700">
      ...
      <!-- Stats Bar -->
      <div
        class="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400 px-2 py-1.5 bg-gray-50 dark:bg-gray-750 rounded"
      >
        <span>总计 {{ stats.total }} 个任务</span>
        <span class="flex items-center gap-3">
          <span
            >待处理: <strong>{{ stats.pending }}</strong></span
          >
          <span
            >进行中: <strong>{{ stats.inProgress }}</strong></span
          >
          <span
            >已完成: <strong>{{ stats.completed }}</strong></span
          >
          <span
            >平均进度: <strong>{{ stats.avgProgress }}%</strong></span
          >
        </span>
      </div>
    </div>
    ...
    <div
      ...
      class="px-4 py-2 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-200 dark:border-blue-800"
    >
      ...
      <div class="flex gap-4 h-full min-w-max">
        ...
        >
          ...
          >
              ...
              <!-- Title -->
              <h4 class="font-semibold text-sm text-gray-900 dark:text-white mb-1 ml-6">
                {{ task.title }}
              </h4>

              <!-- Description -->
              <p class="text-xs text-gray-600 dark:text-gray-400 mb-3 line-clamp-2 ml-6">
                {{ task.description }}
              </p>
              ...
              <!-- Footer -->
              <div
                class="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-600 ml-6"
              >
              ...
              <div
                v-if="showDependencies && task.dependencies && task.dependencies.length > 0"
                class="mt-2 pt-2 border-t border-dashed border-gray-200 dark:border-gray-600 ml-6"
              >
    ...
    <!-- Footer Actions -->
    <div
      class="p-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between"
    >
      <label
        class="flex items-center gap-2 cursor-pointer text-xs text-gray-600 dark:text-gray-400"
      >
        <input v-model="showDependencies" type="checkbox" class="rounded" />
        <span>显示依赖关系</span>
      </label>
      <span class="text-xs text-gray-500 dark:text-gray-400"> 💡 拖拽任务卡片可更改状态 </span>
    </div>
  </div>
</template>
