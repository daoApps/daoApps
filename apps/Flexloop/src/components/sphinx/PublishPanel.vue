...
const platforms: {
  id: 'local' | 'vercel' | 'netlify' | 'custom';
  name: string;
  icon: string;
  desc: string;
}[] = [
...
<template>
  <div class="space-y-6">
    ...
    <!-- 发布状态和按钮 -->
    <div
      class="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-xl p-6 border border-blue-200 dark:border-blue-800"
    >
      <!-- 构建进度条 -->
      ...
      <!-- 部署状态 -->
      <div
        v-if="publishStatus === 'deploying'"
        class="mb-4 p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg"
      >
      ...
      <!-- 部署成功 URL -->
      <div
        v-if="publishStatus === 'success' && deployUrl"
      ...
      <div class="flex items-center justify-between">
        ...
        <button
          :disabled="
            publishStatus === 'building' || publishStatus === 'deploying' || !domain.trim()
          "
          :class="[
          ...
          <div class="flex items-center space-x-3">
            <span
              :class="[
                ...
                getStatusConfig(record.status).bg,
