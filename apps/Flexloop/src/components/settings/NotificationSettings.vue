    ...
    <div class="space-y-8">
          ...
          <table class="w-full text-sm border-collapse">
            <thead>
              <tr class="bg-gray-50 dark:bg-gray-700/50">
                <th
                  class="text-left py-3 px-4 font-medium text-gray-600 dark:text-gray-400 rounded-tl-lg"
                >
                  通知类型
                </th>
                <th class="text-center py-3 px-4 font-medium text-gray-600 dark:text-gray-400">
                  浏览器推送
                </th>
                <th class="text-center py-3 px-4 font-medium text-gray-600 dark:text-gray-400">
                  应用内
                ...
                <th
                  class="text-center py-3 px-4 font-medium text-gray-600 dark:text-gray-400 rounded-tr-lg"
                >
                  短信
                </th>
              </tr>
            ...
            <tbody>
              <tr
                v-for="(_channels, key) in config.channels"
                :key="key"
              ...
              >
                ...
                <td class="py-3 px-2 text-center">
                  <button
                    :class="[
                      'relative w-10 h-5.5 rounded-full transition-colors inline-flex',
                      config.channels[key].browserPush ? 'bg-indigo-500' : 'bg-gray-300'
                    ]"
                  ...
                  >
                    <span
                      :class="[
                        ...
                        config.channels[key].browserPush ? 'left-[18px]' : 'left-0.5'
                      ]"
                    ></span>
                  </button>
                </td>
                ...
                <td class="py-3 px-2 text-center">
                  <button
                    :class="[
                      'relative w-10 h-5.5 rounded-full transition-colors inline-flex',
                      config.channels[key].inApp ? 'bg-indigo-500' : 'bg-gray-300'
                    ]"
                  ...
                  >
                    <span
                      :class="[
                        'absolute top-0.5 w-4.5 h-4.5 bg-white rounded-full shadow transition-transform',
                        config.channels[key].inApp ? 'left-[18px]' : 'left-0.5'
                      ]"
                    ></span>
                  </button>
                </td>
                ...
                <td class="py-3 px-2 text-center">
                  <button
                    v-if="'email' in config.channels[key]"
                    :class="[
                      'relative w-10 h-5.5 rounded-full transition-colors inline-flex',
                      (config.channels[key] as any).email ? 'bg-indigo-500' : 'bg-gray-300'
                    ]"
                    @click="
                      (config.channels[key] as any).email = !(config.channels[key] as any).email
                    "
                  >
                    <span
                      :class="[
                        'absolute top-0.5 w-4.5 h-4.5 bg-white rounded-full shadow transition-transform',
                        (config.channels[key] as any).email ? 'left-[18px]' : 'left-0.5'
                      ]"
                    ></span>
                  </button>
                ...
                <td class="py-3 px-2 text-center">
                  <button
                    v-if="'sms' in config.channels[key]"
                    :class="[
                      'relative w-10 h-5.5 rounded-full transition-colors inline-flex',
                      (config.channels[key] as any).sms ? 'bg-indigo-500' : 'bg-gray-300'
                    ]"
                  ...
                  >
                    <span
                      :class="[
                        'absolute top-0.5 w-4.5 h-4.5 bg-white rounded-full shadow transition-transform',
                        (config.channels[key] as any).sms ? 'left-[18px]' : 'left-0.5'
                      ]"
                    ></span>
                  </button>
                  <span v-else class="text-gray-300 dark:text-gray-600">—</span>
      ...
      <div>
        ...
        <div class="flex items-center gap-3">
          <input
            v-model="config.doNotDisturbStart"
            type="time"
          ...
          <input
            v-model="config.doNotDisturbEnd"
            type="time"
            class="px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-400 outline-none"
          />
          <span class="text-sm text-gray-500 whitespace-nowrap"
            >{{ config.doNotDisturbStart }} - {{ config.doNotDisturbEnd }}</span
          >
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          >
            ...
            <svg
              v-if="config.sound === sound.id"
              class="w-4 h-4 text-indigo-500 ml-auto"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
        ...
        <button
          :class="[
            ...
            config.vibration ? 'bg-indigo-500' : 'bg-gray-300'
          ]"
        ...
        >
          <span
            :class="[
              ...
              config.vibration ? 'left-[22px]' : 'left-0.5'
            ]"
          ></span>
        </button>
      </div>
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
        <button
          class="px-5 py-2.5 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white border border-gray-300 dark:border-gray-600 rounded-lg transition-colors"
          @click="resetToDefault"
        >
          重置默认
        </button>
        <button
          class="px-6 py-2.5 text-sm bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors font-medium shadow-md"
