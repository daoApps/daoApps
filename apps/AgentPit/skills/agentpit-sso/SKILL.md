---
name: "agentpit-sso"
description: "为 AgentPit 子应用生成 OAuth2 SSO 单点登录功能模块，包含后端 OAuth 端点、前端回调页和防循环机制。当用户需要集成 AgentPit 平台授权登录、实现单点登录或添加第三方 OAuth 认证时调用此技能。"
---

# AgentPit SSO 单点登录功能生成器

## 概述

此技能用于在 AgentPit 项目中快速集成 **AgentPit 平台 OAuth2 单点登录（SSO）** 功能，提供完整的认证流程实现。
...
// ✅ 安全的配置读取方式（推荐）
function getOAuthConfig(): AgentPitOAuthConfig {
  return {
    authorizationUrl: process.env.AGENTPIT_OAUTH_AUTH_URL || 'https://pagent.agentpit.io/oauth2/authorize',
    tokenUrl: process.env.AGENTPIT_OAUTH_TOKEN_URL || 'https://pagent.agentpit.io/oauth2/token',
    callbackUrl: process.env.AGENTPIT_OAUTH_CALLBACK_URL || 'https://pagent.agentpit.io/api/auth/agentpit/callback',
    // 🔒 关键：从环境变量读取，fallback 为占位符
    clientId: process.env.AGENTPIT_OAUTH_CLIENT_ID || '[CLIENT_ID]',
    clientSecret: process.env.AGENTPIT_OAUTH_CLIENT_SECRET || '[CLIENT_SECRET]',
    providerName: 'agentpit',
    loginButtonText: 'agentpit 授权登陆',
...
# OAuth2 提供商信息（必填）
# 从 AgentPit 平台获取：https://pagent.agentpit.io/console
AGENTPIT_OAUTH_CLIENT_ID=[CLIENT_ID]
AGENTPIT_OAUTH_CLIENT_SECRET=[CLIENT_SECRET]

# OAuth2 端点地址（通常不需要修改）
AGENTPIT_OAUTH_AUTH_URL=https://pagent.agentpit.io/oauth2/authorize
AGENTPIT_OAUTH_TOKEN_URL=https://pagent.agentpit.io/oauth2/token
AGENTPIT_OAUTH_CALLBACK_URL=https://pagent.agentpit.io/api/auth/agentpit/callback

# 应用回调地址（你的应用接收 OAuth 回调的 URL）
NEXT_PUBLIC_APP_CALLBACK_URL=http://localhost:3000/api/auth/callback/agentpit
# 生产环境: https://your-app.com/api/auth/callback/agentpit
...
AGENTPIT_OAUTH_STATE_SECRET=[STATE_SECRET]

# 最大允许的重定向次数（防循环）
AGENTPIT_MAX_REDIRECT_COUNT=5
```
...
```typescript
/**
 * AgentPit OAuth2 类型定义
 */

export interface AgentPitOAuthConfig {
  clientId: string
  clientSecret: string
  authorizationUrl: string
  tokenUrl: string
...
```typescript
import type { AgentPitOAuthConfig, OAuthState, OAuthTokenResponse, CallbackParams } from '@/types/oauth'
import { generateRandomString, sha256 } from '@/utils/crypto'

class AgentPitOAuthService {
  private config: AgentPitOAuthConfig

  constructor() {
    // ✅ 安全的写法（必须使用）
    this.config = {
      clientId: process.env.AGENTPIT_OAUTH_CLIENT_ID!,
      clientSecret: process.env.AGENTPIT_OAUTH_CLIENT_SECRET!,  // 注意：修正拼写错误
      authorizationUrl: process.env.AGENTPIT_OAUTH_AUTH_URL!,
      tokenUrl: process.env.AGENTPIT_OAUTH_TOKEN_URL!,
      callbackUrl: process.env.AGENTPIT_OAUTH_CALLBACK_URL!,
      scope: ['openid', 'profile', 'email'],
    }

    // 添加运行时验证
    if (!this.config.clientId || this.config.clientId === '[CLIENT_ID]') {
      throw new Error('Missing required environment variable: AGENTPIT_OAUTH_CLIENT_ID')
    }
    if (!this.config.clientSecret || this.config.clientSecret === '[CLIENT_SECRET]') {
      throw new Error('Missing required environment variable: AGENTPIT_OAUTH_CLIENT_SECRET')
    }
  }
   ...
   * 使用访问令牌获取用户信息
   */
  async getUserInfo(accessToken: string): Promise<AgentPitUser> {
    const userInfoUrl = 'https://pagent.agentpit.io/api/userinfo' // 根据实际 API 调整
  ...
  private getCallbackUrl(): string {
    return process.env.NEXT_PUBLIC_APP_CALLBACK_URL || `${window.location.origin}/api/auth/callback/agentpit`
  }
}

export const agentPitOAuthService = new AgentPitOAuthService()
```

#### 2.2 API 路由 - 发起授权

**文件**: `src/pages/api/auth/login/agentpit.ts` (Pages Router) 或 `src/app/api/auth/login/agentpit/route.ts` (App Router)

```typescript
import type { NextApiRequest, NextApiResponse } from 'next'
import { agentPitOAuthService } from '@/services/oauth/agentpit-oauth'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 生成授权 URL 和 state
    const { url, state } = await agentPitOAuthService.getAuthorizationUrl(redirectUri)
...
**文件**: `src/pages/api/auth/callback/agentpit.ts` (Pages Router) 或 `src/app/api/auth/callback/agentpit/route.ts` (App Router)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 处理 OAuth 回调
    const tokens = await agentPitOAuthService.handleCallback(
      { code, state, error, error_description },
      storedState
    )

    // 获取用户信息
    const user = await agentPitOAuthService.getUserInfo(tokens.access_token)
...
**文件**: `src/components/auth/AgentPitLoginButton.vue` (Vue) 或 `src/components/auth/AgentPitLoginButton.tsx` (React/Next.js)
...
const handleLogin = async () => {
  try {
    ...
    window.location.href = `/api/auth/login/agentpit?${params.toString()}`
...
onMounted(async () => {
  try {
    ...
    if (code && state) {
      ...
      const response = await fetch(`/api/auth/callback/agentpit?code=${code}&state=${state}`, {
        method: 'GET',
        credentials: 'include',
      })
...
<script setup lang="ts">
import AgentPitLoginButton from '@/components/auth/AgentPitLoginButton.vue'

const onLoginSuccess = (user: AgentPitUser) => {
...
// 直接跳转到 OAuth 登录
function loginWithAgentPit() {
  window.location.href = `/api/auth/login/agentpit?redirect=/settings`
}
