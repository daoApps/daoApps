...
const agentSpecificFields = computed(() => {
  ...
  const fieldMap: Record<string, { key: string; label: string; type: 'text' | 'number' | 'select'; options?: string[]; defaultValue: any }[]> = {
    'agent-planner': [
      { key: 'planningDepth', label: '规划深度', type: 'select', options: ['shallow', 'medium', 'deep'], defaultValue: 'medium' },
      { key: 'autoDecompose', label: '自动分解任务', type: 'select', options: ['yes', 'no'], defaultValue: 'yes' }
    ],
    'agent-writer': [
      { key: 'writingStyle', label: '写作风格', type: 'select', options: ['professional', 'casual', 'creative'], defaultValue: 'professional' },
      { key: 'maxWords', label: '最大字数', type: 'number', defaultValue: 2000 },
      { key: 'includeSEO', label: 'SEO优化', type: 'select', options: ['yes', 'no'], defaultValue: 'yes' }
    ],
    'agent-coder': [
      { key: 'language', label: '首选语言', type: 'select', options: ['typescript', 'python', 'java', 'go'], defaultValue: 'typescript' },
      { key: 'codeStyle', label: '代码风格', type: 'select', options: ['strict', 'moderate', 'loose'], defaultValue: 'moderate' },
      { key: 'enableTests', label: '自动生成测试', type: 'select', options: ['yes', 'no'], defaultValue: 'yes' }
    ],
    'agent-researcher': [
      { key: 'searchDepth', label: '搜索深度', type: 'select', options: ['quick', 'standard', 'thorough'], defaultValue: 'standard' },
      { key: 'sourcesCount', label: '来源数量', type: 'number', defaultValue: 10 },
      { key: 'verifySources', label: '验证来源', type: 'select', options: ['yes', 'no'], defaultValue: 'yes' }
    ],
    'agent-designer': [
      { key: 'designSystem', label: '设计系统', type: 'select', options: ['material', 'ant-design', 'custom'], defaultValue: 'material' },
      { key: 'responsive', label: '响应式设计', type: 'select', options: ['yes', 'no'], defaultValue: 'yes' },
      { key: 'accessibility', label: '无障碍支持', type: 'select', options: ['wcag-a', 'wcag-aa', 'wcag-aaa'], defaultValue: 'wcag-aa' }
    ],
    'agent-analyst': [
      { key: 'analysisMethod', label: '分析方法', type: 'select', options: ['quantitative', 'qualitative', 'mixed'], defaultValue: 'mixed' },
      { key: 'visualizationType', label: '可视化类型', type: 'select', options: ['charts', 'tables', 'dashboards'], defaultValue: 'charts' }
    ],
    'agent-translator': [
      { key: 'targetLanguage', label: '目标语言', type: 'select', options: ['english', 'japanese', 'korean', 'french', 'german'], defaultValue: 'english' },
      { key: 'preserveFormat', label: '保留格式', type: 'select', options: ['yes', 'no'], defaultValue: 'yes' },
      { key: 'domainGlossary', label: '领域术语表', type: 'text', defaultValue: '' }
    ],
    'agent-consultant': [
      { key: 'consultingFocus', label: '咨询重点', type: 'select', options: ['strategy', 'operations', 'finance', 'marketing'], defaultValue: 'strategy' },
      { key: 'riskTolerance', label: '风险容忍度', type: 'select', options: ['low', 'medium', 'high'], defaultValue: 'medium' },
...
<template>
  <div class="h-full flex flex-col bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              :class="validationErrors.timeout ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'"
              @change="updateConfig"
            />
            <p v-if="validationErrors.timeout" class="mt-1 text-xs text-red-500">{{ validationErrors.timeout }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              max="10"
              class="w-full px-3 py-2 text-sm border rounded-lg bg-gray-50 dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="validationErrors.retryCount ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'"
              @change="updateConfig"
            />
            <p v-if="validationErrors.retryCount" class="mt-1 text-xs text-red-500">{{ validationErrors.retryCount }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              @input="updateConfig"
            />
            <p v-if="validationErrors.priorityWeight" class="mt-1 text-xs text-red-500">{{ validationErrors.priorityWeight }}</p>
          </div>
        </div>
      </section>
    ...
    <div class="border-t border-gray-200 dark:border-gray-700 p-4">
      ...
      <p v-else class="text-xs text-gray-500 dark:text-gray-400 text-center py-2">
        暂无保存的预设
      </p>
    </div>
  </div>
