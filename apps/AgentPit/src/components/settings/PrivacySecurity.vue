  ...
  <div class="privacy-security">
    ...
    <div class="flex gap-2 mb-6 border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
      <button v-for="tab in [
        ...
        { id: 'data', label: '数据与账户' },
      ]" :key="tab.id" @click="activeSection = tab.id"
        :class="[
    ...
    <div v-if="activeSection === 'password'" class="space-y-5">
      <div>
        ...
        <div class="relative">
          <input :type="passwordForm.showCurrent ? 'text' : 'password'" v-model="passwordForm.currentPassword"
            placeholder="输入当前密码"
          ...
          <button @click="passwordForm.showCurrent = !passwordForm.showCurrent" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
      ...
      <div>
        ...
        <div class="relative">
          <input :type="passwordForm.showNew ? 'text' : 'password'" v-model="passwordForm.newPassword"
            placeholder="输入新密码（至少8位）"
            class="w-full px-3 py-2.5 pr-10 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-400 outline-none" />
          <button @click="passwordForm.showNew = !passwordForm.showNew" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
      ...
      <div>
        ...
        <div class="relative">
          <input :type="passwordForm.showConfirm ? 'text' : 'password'" v-model="passwordForm.confirmPassword"
            placeholder="再次输入新密码"
          ...
          <button @click="passwordForm.showConfirm = !passwordForm.showConfirm" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
      ...
      <button @click="changePassword" :disabled="!passwordForm.currentPassword || !passwordForm.newPassword || !passwordsMatch"
        class="px-6 py-2.5 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed">
        修改密码
      </button>
    </div>
    ...
    <div v-if="activeSection === 'twofa'" class="space-y-6">
      ...
      <div v-if="twoFactorEnabled && totpStep > 0" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 space-y-4">
        ...
        <div v-if="totpStep === 1">
          ...
          <button @click="totpStep = 2" class="w-full mt-4 py-2.5 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors font-medium">
            我已完成扫描 →
          </button>
        </div>
        ...
        <div v-if="totpStep === 2">
          ...
          <button @click="verifyTOTP" :disabled="totpCode.length !== 6"
            class="w-full mt-4 py-2.5 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors font-medium disabled:opacity-50">
            验证并激活
          </button>
        </div>
        ...
        <div v-if="totpStep === 3">
          ...
          <div class="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-200 dark:border-yellow-800">
            <div class="flex items-center justify-between mb-2">
              ...
              <button @click="regenerateBackupCodes" class="text-xs text-yellow-600 hover:text-yellow-800 underline">重新生成</button>
            </div>
            <p class="text-xs text-yellow-600 dark:text-yellow-400 mb-3">请妥善保存以下备用码，每个码只能使用一次。当无法使用常规2FA方式时可用于登录。</p>
            <div class="grid grid-cols-2 gap-2">
              <code v-for="(code, idx) in backupCodes" :key="idx"
    ...
    <div v-if="activeSection === 'devices'" class="space-y-4">
      <div v-for="device in deviceList" :key="device.id"
        ...
        <button v-if="!device.isCurrentDevice" @click="forceLogout(device.id)"
          class="px-3 py-1.5 text-sm text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors flex-shrink-0">
          强制下线
        </button>
      </div>
    </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full p-6">
            <div class="text-center">
              ...
              <div class="flex gap-3">
                <button @click="showDeleteConfirm = false" class="flex-1 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  取消
                </button>
                <button @click="deleteAccount" class="flex-1 py-2.5 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors font-bold">
                  确认注销
                </button>
              </div>
            </div>
