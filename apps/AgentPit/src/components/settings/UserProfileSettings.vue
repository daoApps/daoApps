...
const isFormValid = computed(() => nicknameValid.value && emailValid.value && phoneValid.value && bioValid.value)

watch(() => JSON.stringify(profile), () => {
  ...
  <div class="user-profile-settings">
    ...
    <div class="flex items-center gap-5 mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
      <div class="relative group">
        <div class="w-20 h-20 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center overflow-hidden cursor-pointer ring-4 ring-white dark:ring-gray-800 shadow-lg">
...
v-model="profile.nickname" type="text" placeholder="请输入昵称（2-30字符）"
        ...
        <p class="text-xs mt-1" :class="nicknameValid ? 'text-gray-500' : 'text-red-500'">
          {{ profile.nickname.length }}/30 字符 {{ nicknameValid ? '' : '- 昵称长度需在2-30个字符之间' }}
        </p>
      </div>
...
v-model="profile.email" type="email" placeholder="example@email.com"
          ...
          <p v-if="!emailValid && profile.email" class="text-xs mt-1 text-red-500">请输入有效的邮箱地址</p>
        </div>
        <div>
...
v-model="profile.phone" type="tel" placeholder="13800138000"
          ...
          <p v-if="!phoneValid && profile.phone" class="text-xs mt-1 text-red-500">请输入有效的手机号</p>
        </div>
      </div>
