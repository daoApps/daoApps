      ...
      <Transition name="fade" mode="out-in">
        <div v-if="mode === 'subscription'" key="sub" class="mt-6 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 space-y-4">
          <div class="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" v-model.number="pricing.monthlyPrice" min="0" step="1" placeholder="0"
            ...
            <div>
              ...
              <input type="number" v-model.number="pricing.quarterlyPrice" min="0" step="1" placeholder="0"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">年付价格 (¥)</label>
              <input type="number" v-model.number="pricing.yearlyPrice" min="0" step="1" placeholder="0"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
            </div>
          </div>
        </div>
        ...
        <div v-else-if="mode === 'payPerUse'" key="ppu" class="mt-6 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
          ...
          <div class="flex items-center gap-3">
            <input type="number" v-model.number="pricing.perUsePrice" min="0" step="0.01" placeholder="0.00"
        ...
        <div v-else-if="mode === 'membership'" key="mem" class="mt-6 space-y-3">
          <div v-for="(level, idx) in membershipLevels" :key="idx" class="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
            <div class="flex items-center justify-between mb-3">
              <input type="text" v-model="level.name" placeholder="等级名称"
              ...
              <div class="flex items-center gap-2">
                ...
                <input type="number" v-model.number="level.price" min="0" step="1" class="w-20 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded bg-transparent text-right focus:ring-1 focus:ring-blue-500 outline-none" @input="emitUpdate" />
                <span class="text-xs text-gray-400">/月</span>
                <button v-if="membershipLevels.length > 1" type="button" @click="removeMembershipLevel(idx)" class="text-red-400 hover:text-red-600 p-1" aria-label="删除等级">×</button>
              </div>
            </div>
            <div class="space-y-1.5">
              <div v-for="(feature, fIdx) in level.features" :key="fIdx" class="flex items-center gap-2">
                <span class="text-green-500 text-xs">✓</span>
                <input type="text" :value="feature" @input="updateMembershipFeature(idx, fIdx, ($event.target as HTMLInputElement).value)"
                  placeholder="功能描述..." class="flex-1 text-xs px-2 py-1 border border-gray-200 dark:border-gray-600 rounded bg-gray-50 dark:bg-gray-700 outline-none focus:ring-1 focus:ring-blue-500" />
                <button type="button" @click="removeMembershipFeature(idx, fIdx)" class="text-gray-400 hover:text-red-500 text-xs">×</button>
              </div>
              <button type="button" @click="addMembershipFeature(idx)" class="text-xs text-blue-500 hover:text-blue-600 pl-4">+ 添加功能</button>
            </div>
          </div>
          <button type="button" @click="addMembershipLevel" class="w-full py-2 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-sm text-gray-500 hover:text-blue-500 hover:border-blue-400 transition-colors">
            + 添加会员等级
          </button>
        </div>
      </Transition>
      ...
      <div class="space-y-4 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
        <div>
          ...
          <div class="flex flex-wrap gap-2 mb-2">
            <span v-for="(slot, idx) in serviceLimits.availableSlots" :key="idx"
              ...
              {{ slot.start }} - {{ slot.end }}
              <button type="button" @click="removeTimeSlot(idx)" class="hover:text-red-500 ml-0.5" aria-label="删除时段">×</button>
            </span>
          </div>
          <div class="flex items-center gap-2">
            <input type="time" v-model="newSlotStart" class="px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 outline-none" />
            <span class="text-gray-400">至</span>
            <input type="time" v-model="newSlotEnd" class="px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 outline-none" />
            <button type="button" @click="addTimeSlot" class="px-3 py-1.5 bg-blue-500 text-white text-xs rounded-lg hover:bg-blue-600 transition-colors">添加时段</button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          <div>
            ...
            <input type="range" v-model.number="serviceLimits.maxConcurrentUsers" min="1" max="100" step="1"
          ...
          <div>
            ...
            <input type="number" v-model.number="serviceLimits.dailyRequestLimit" min="0" step="100"
          ...
          <div>
            ...
            <input type="number" v-model.number="serviceLimits.apiRateLimit" min="1" step="10"
              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm focus:ring-2 focus:ring-green-500 outline-none" />
          </div>
        </div>
      </div>
    ...
    <section>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
        <div>
          ...
          <input type="range" v-model.number="platformCommission" min="0" max="50" step="1"
        ...
        <div>
          ...
          <input type="number" v-model.number="trialDays" min="0" max="90" step="1"
