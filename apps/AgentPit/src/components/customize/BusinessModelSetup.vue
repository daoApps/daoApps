...
const serviceLimits = ref({
  availableSlots: props.modelValue.serviceLimits.availableSlots || [
...
const estimatedMonthlyRevenue = computed(() => {
  ...
  switch (mode.value) {
    ...
    case 'membership': {
      const avgPrice =
        membershipLevels.value.reduce((sum, l) => sum + l.price, 0) /
        (membershipLevels.value.length || 1);
...
const emitUpdate = () => {
  emit('update:modelValue', {
    ...
    trialSettings:
      trialDays.value > 0 ? { days: trialDays.value, featureLimits: ['高级功能受限'] } : undefined
  });
};

watch(
  [mode, pricing, serviceLimits, platformCommission, trialDays],
  () => {
    emitUpdate();
  },
  { deep: true }
  ...
  <div class="space-y-8">
    <section>
      ...
      <Transition name="fade" mode="out-in">
        <div
          v-if="mode === 'subscription'"
          key="sub"
          class="mt-6 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 space-y-4"
        >
          <div class="grid grid-cols-3 gap-4">
            <div>
              ...
              >
              <input
                v-model.number="pricing.monthlyPrice"
                type="number"
                min="0"
                step="1"
                placeholder="0"
            ...
            <div>
              ...
              >
              <input
                v-model.number="pricing.quarterlyPrice"
                type="number"
                min="0"
                step="1"
                placeholder="0"
            ...
            <div>
              ...
              >
              <input
                v-model.number="pricing.yearlyPrice"
                type="number"
                min="0"
                step="1"
                placeholder="0"
        ...
        <div
          v-else-if="mode === 'payPerUse'"
          key="ppu"
          class="mt-6 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50"
        >
          <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1"
            >单次调用价格 (¥)</label
          >
          <div class="flex items-center gap-3">
            <input
              v-model.number="pricing.perUsePrice"
              type="number"
              min="0"
              step="0.01"
              placeholder="0.00"
        ...
        <div v-else-if="mode === 'membership'" key="mem" class="mt-6 space-y-3">
          <div
            v-for="(level, idx) in membershipLevels"
            :key="idx"
          ...
          >
            <div class="flex items-center justify-between mb-3">
              <input
                ...
                type="text"
                placeholder="等级名称"
                class="font-medium text-sm bg-transparent border-b border-transparent hover:border-gray-300 focus:border-blue-500 outline-none px-1 py-0.5 text-gray-800 dark:text-gray-200"
                @input="emitUpdate"
              />
              <div class="flex items-center gap-2">
                ...
                <input
                  ...
                  min="0"
                  step="1"
                  class="w-20 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded bg-transparent text-right focus:ring-1 focus:ring-blue-500 outline-none"
                  @input="emitUpdate"
                />
                <span class="text-xs text-gray-400">/月</span>
                <button
                  v-if="membershipLevels.length > 1"
                  type="button"
                  class="text-red-400 hover:text-red-600 p-1"
                  aria-label="删除等级"
                  @click="removeMembershipLevel(idx)"
                >
                  ×
                </button>
              </div>
            ...
            <div class="space-y-1.5">
              <div
                v-for="(feature, fIdx) in level.features"
                :key="fIdx"
              ...
              >
                ...
                <input
                  ...
                  :value="feature"
                  placeholder="功能描述..."
                  class="flex-1 text-xs px-2 py-1 border border-gray-200 dark:border-gray-600 rounded bg-gray-50 dark:bg-gray-700 outline-none focus:ring-1 focus:ring-blue-500"
                  @input="
                    updateMembershipFeature(idx, fIdx, ($event.target as HTMLInputElement).value)
                  "
                />
                <button
                  type="button"
                  class="text-gray-400 hover:text-red-500 text-xs"
                  @click="removeMembershipFeature(idx, fIdx)"
                >
                  ×
                </button>
              </div>
              <button
                type="button"
                class="text-xs text-blue-500 hover:text-blue-600 pl-4"
                @click="addMembershipFeature(idx)"
              >
                + 添加功能
              </button>
            </div>
          ...
          <button
            type="button"
            class="w-full py-2 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-sm text-gray-500 hover:text-blue-500 hover:border-blue-400 transition-colors"
            @click="addMembershipLevel"
          >
            + 添加会员等级
          </button>
        </div>
      ...
      <div class="space-y-4 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
        <div>
          ...
          <div class="flex flex-wrap gap-2 mb-2">
            <span
              v-for="(slot, idx) in serviceLimits.availableSlots"
              :key="idx"
          ...
          <div class="flex items-center gap-2">
            <input
              ...
              type="time"
              class="px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 outline-none"
            />
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          <div>
            ...
            <input
              v-model.number="serviceLimits.maxConcurrentUsers"
              type="range"
              min="1"
              max="100"
              step="1"
          ...
          <div>
            ...
            >
            <input
              v-model.number="serviceLimits.dailyRequestLimit"
              type="number"
              min="0"
              step="100"
          ...
          <div>
            ...
            >
            <input
              v-model.number="serviceLimits.apiRateLimit"
              type="number"
              min="1"
              step="10"
              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm focus:ring-2 focus:ring-green-500 outline-none"
    ...
    <section>
      ...
      <div
        class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-xl border border-gray-200 dark:border-gray-700"
      >
        <div>
          <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
          ...
          <input
            v-model.number="platformCommission"
            type="range"
            min="0"
            max="50"
            step="1"
    ...
    <section
      class="p-5 rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 border border-emerald-200 dark:border-emerald-800"
    >
      <h3
        class="text-base font-semibold text-emerald-800 dark:text-emerald-300 mb-3 flex items-center gap-2"
      >
        📈 预估月收入
      </h3>
      <div class="flex items-end gap-2">
        <span class="text-3xl font-bold text-emerald-600 dark:text-emerald-400"
          >¥{{
            estimatedMonthlyRevenue.toLocaleString('zh-CN', { maximumFractionDigits: 0 })
          }}</span
        >
