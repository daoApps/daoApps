...
const currentTheme = computed(() =>
  themeColors.find((t) => t.id === props.config.appearance.themeId)
);
const primaryColor = computed(
  () => props.config.appearance.customColors?.primary || currentTheme.value?.primary || '#3B82F6'
);
const secondaryColor = computed(
  () =>
    props.config.appearance.customColors?.secondary || currentTheme.value?.secondary || '#60A5FA'
);
...
const textColor = computed(() =>
  props.config.appearance.darkMode ? '#e5e7eb' : currentTheme.value?.text || '#1E40AF'
);
const titleFontFamily = computed(
  () => fontOptions.find((f) => f.id === props.config.appearance.titleFont)?.family || 'system-ui'
);
const bodyFontFamily = computed(
  () => fontOptions.find((f) => f.id === props.config.appearance.bodyFont)?.family || 'system-ui'
);
...
<template>
  <div class="flex flex-col h-full" :class="{ 'fixed inset-0 z-50 bg-gray-900': isFullscreen }">
    ...
    <div
      class="flex-1 flex items-start justify-center overflow-auto p-2"
      :class="{ 'p-6': isFullscreen }"
    >
      <div
        :key="previewKey"
        class="transition-all duration-300 ease-out overflow-hidden relative"
        :style="{
          ...
          color: textColor,
          boxShadow:
            config.appearance.shadowIntensity !== 'none'
              ? `0 ${config.appearance.shadowIntensity === 'heavy' ? '25' : config.appearance.shadowIntensity === 'medium' ? '12' : '4'}px rgba(0,0,0,${config.appearance.darkMode ? '0.4' : '0.1'})`
              : 'none',
      ...
      >
        <div class="h-full overflow-y-auto custom-preview-scrollbar">
          <div class="p-5 sm:p-6 space-y-5">
            <header
              class="flex items-center gap-4 pb-4 border-b"
              :style="{ borderColor: config.appearance.darkMode ? '#374151' : primaryColor + '20' }"
            >
              <div
              ...
              >
                {{
                  config.basicInfo.avatarId
                    ? (() => {
                        const avatar = avatarLibrary.find(
                          (a: any) => a.id === config.basicInfo.avatarId
                        );
                        return avatar?.emoji || '🤖';
                      })()
                    : config.basicInfo.customAvatar
                      ? '🖼️'
                      : '🤖'
                }}
              </div>
              <div class="flex-1 min-w-0">
                <h2
                  ...
                  :style="{
                    ...
                    color: textColor,
                    fontSize: `${config.appearance.fontSize * 1.25}px`
                  }"
                >
                  {{ config.basicInfo.name || '未命名智能体' }}
                </h2>
                <p class="text-xs mt-0.5 opacity-70 line-clamp-2">
                  {{ config.basicInfo.description || '暂无描述信息...' }}
                </p>
              </div>
              <span
            ...
            <section>
              <h3
                ...
                :style="{
                  fontFamily: titleFontFamily,
                  color: textColor,
                  fontSize: `${config.appearance.fontSize * 1.05}px`
                }"
              >
                能力配置
              </h3>
              <div v-if="enabledAbilitiesList.length" class="space-y-2">
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div
                      ...
                      :style="{
                        backgroundColor: config.appearance.darkMode ? '#374151' : '#00000010'
                      }"
                    >
                      <div
                        class="h-full rounded-full transition-all"
                        :style="{
                          width: `${item.config.proficiency}%`,
                ...
                <p
                  v-if="enabledAbilitiesList.length > 6"
                  class="text-[11px] opacity-50 text-center pt-1"
                >
            ...
            <section>
              ...
              <div class="grid grid-cols-2 gap-2.5">
                <div
                  ...
                  :style="{
                    backgroundColor: config.appearance.darkMode ? '#1f2937' : '#ffffff50',
                    borderRadius: `${config.appearance.borderRadius * 0.8}px`
                  }"
                >
                  <p class="text-[10px] opacity-60 mb-0.5">定价模式</p>
                  <p class="font-bold text-sm" :style="{ color: primaryColor }">
                    {{ modeLabels[config.businessModel.mode] || '免费' }}
                  </p>
                </div>
                <div
                  class="p-3 rounded-lg text-center"
                ...
                >
                  ...
                  <p class="font-bold text-sm" :style="{ color: primaryColor }">
                    {{ config.businessModel.serviceLimits.dailyRequestLimit.toLocaleString() }}
                  </p>
                </div>
                <div
                  class="p-3 rounded-lg text-center"
                ...
                >
                  ...
                  <p class="font-bold text-sm" :style="{ color: primaryColor }">
                    {{ config.businessModel.serviceLimits.maxConcurrentUsers }}
                  </p>
                </div>
                <div
                  class="p-3 rounded-lg text-center"
                ...
                >
                  ...
                  <p class="font-bold text-sm" :style="{ color: primaryColor }">
                    {{ config.businessModel.platformCommission }}%
                  </p>
                </div>
              </div>
            </section>
