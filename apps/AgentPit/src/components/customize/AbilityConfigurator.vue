...
import { abilities as allAbilities, abilityPresets, type AgentConfig } from '../../data/mockCustomize'
...
const enabledAbilities = ref<Record<string, { enabled: boolean; proficiency: number; params: Record<string, unknown> }>>(
...
const totalEnabled = computed(() => Object.values(enabledAbilities.value).filter(a => a.enabled).length)
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <span class="text-sm text-gray-500 dark:text-gray-400">
        已选择 <span class="font-bold text-blue-600 dark:text-blue-400">{{ totalEnabled }}</span> / {{ allAbilities.length }} 个技能
      </span>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          >
            <div class="flex items-center gap-2.5">
              ...
              <span class="font-medium text-gray-800 dark:text-gray-200">{{ categoryLabels[category] }}</span>
              <span class="text-xs px-1.5 py-0.5 bg-gray-200 dark:bg-gray-700 rounded text-gray-500 dark:text-gray-400">{{ groupAbilities.length }}</span>
              <span v-if="categoryCounts[category]" class="text-xs px-1.5 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded font-medium">
          ...
          <Transition name="expand">
            <div v-show="expandedCategories.has(category)" class="divide-y divide-gray-100 dark:divide-gray-700/50">
              <div
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium text-sm text-gray-800 dark:text-gray-200">{{ ability.name }}</span>
                    <span v-if="ability.isPremium" class="text-[10px] px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded font-medium">PRO</span>
                  </div>
                  <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-1">{{ ability.description }}</p>
                  <div v-if="!isDependencyMet(ability.id)" class="mt-1 flex items-center gap-1">
                    <svg class="w-3 h-3 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    <span class="text-[10px] text-amber-600 dark:text-amber-400">
                      需要：{{ getUnmetDependencies(ability.id).map(d => allAbilities.find(a => a.id === d)?.name).join('、') }}
                    </span>
                  </div>
                </div>
      ...
      <div class="lg:col-span-1">
        <div v-if="selectedAbility && selectedAbilityConfig" class="sticky top-4 border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-white dark:bg-gray-800">
...
class="text-[10px] px-1.5 py-0.5 rounded capitalize"
              :class="{
                ...
                'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400': selectedAbility.category === 'analysis',
                'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400': selectedAbility.category === 'tool',
                'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400': selectedAbility.category === 'multimodal'
              }"
            >{{ categoryLabels[selectedAbility.category] }}</span>
          </div>
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">{{ selectedAbility.description }}</p>

          <div class="space-y-3">
            <p class="text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">参数配置</p>
          ...
          <div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
            <div class="flex items-center justify-between text-xs">
              ...
              <span class="font-bold text-blue-600 dark:text-blue-400">{{ selectedAbilityConfig.proficiency }}%</span>
            </div>
        ...
        <div v-else class="border border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center">
          <span class="text-4xl block mb-2">👆</span>
          <p class="text-sm text-gray-500 dark:text-gray-400">点击左侧技能项<br/>查看和编辑详细参数</p>
        </div>
      </div>
    </div>
  </div>
