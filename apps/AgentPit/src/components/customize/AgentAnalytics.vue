...
use([CanvasRenderer, LineChart, PieChart, BarChart, TitleComponent, TooltipComponent, LegendComponent, GridComponent, GraphicComponent])
...
const getTrendData = () => {
  const ranges: Record<string, typeof data.value.callTrend> = {
    ...
    month: [...data.value.callTrend, ...data.value.callTrend.map(d => ({ date: d.date.replace('04-', '03-'), value: Math.floor(d.value * 0.85) }))].slice(0, 14),
...
const trendOption = computed(() => ({
  ...
  xAxis: { type: 'category', data: getTrendData().map(d => d.date), axisLine: { lineStyle: { color: '#e5e7eb' } }, axisLabel: { color: '#9ca3af', fontSize: 11 } },
...
const sourceOption = computed(() => ({
  ...
  legend: { orient: 'vertical', right: '5%', top: 'center', textStyle: { fontSize: 12, color: '#6b7280' } },
  series: [{
    type: 'pie',
    radius: ['45%', '70%'],
    center: ['35%', '50%'],
...
const questionsOption = computed(() => ({
  ...
  xAxis: { type: 'value', axisLine: { show: false }, splitLine: { lineStyle: { color: '#f3f4f6' } }, axisLabel: { color: '#9ca3af', fontSize: 10 } },
  yAxis: {
    type: 'category',
...
const revenueOption = computed(() => ({
  ...
  xAxis: { type: 'category', data: data.value.revenueTrend.map(d => d.date), axisLabel: { color: '#9ca3af', fontSize: 11 } },
  yAxis: { type: 'value', axisLabel: { color: '#9ca3af', fontSize: 11, formatter: (v: number) => `¥${(v / 1000).toFixed(1)}k` } },
  series: [{
    type: 'line',
...
<template>
  <div class="space-y-6">
      ...
      >
        <div class="flex items-center justify-between mb-3">
          ...
          <span
            ...
            :class="kpi.change >= 0 ? 'bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'"
          >
            {{ kpi.change >= 0 ? '↑' : '↓' }} {{ Math.abs(kpi.change) }}%
          </span>
        </div>
        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ kpi.value().toLocaleString() }}{{ kpi.suffix }}</h3>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700">
      ...
      <div class="space-y-3">
        <div v-for="fb in data.feedbacks" :key="fb.id" class="flex gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-750 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
          <div class="flex gap-0.5 shrink-0 pt-0.5">
            <span v-for="s in 5" :key="s" class="text-sm" :class="s <= fb.rating ? 'text-amber-400' : 'text-gray-300 dark:text-gray-600'">★</span>
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm text-gray-700 dark:text-gray-300">{{ fb.content }}</p>
            <div class="flex items-center gap-2 mt-1.5">
              <span class="text-[10px] text-gray-400">{{ fb.time }}</span>
              <span class="text-[10px] px-1.5 py-0.5 rounded" :class="fb.replied ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'">
                {{ fb.replied ? '已回复' : '待回复' }}
              </span>
            </div>
          </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="text-center p-4 rounded-lg bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/10 dark:to-emerald-900/10">
          <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ data.performance.successRate }}%</p>
          <p class="text-xs text-gray-500 mt-1">成功率</p>
          <div class="mt-2 w-full h-1.5 bg-green-100 dark:bg-green-900/30 rounded-full overflow-hidden">
            <div class="h-full bg-green-500 rounded-full" :style="{ width: `${data.performance.successRate}%` }"></div>
          </div>
        </div>
        <div class="text-center p-4 rounded-lg bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/10 dark:to-rose-900/10">
          <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ data.performance.errorRate }}%</p>
          ...
          <div class="mt-2 w-full h-1.5 bg-red-100 dark:bg-red-900/30 rounded-full overflow-hidden">
            <div class="h-full bg-red-500 rounded-full" :style="{ width: `${data.performance.errorRate}%` }"></div>
          </div>
        </div>
        <div class="text-center p-4 rounded-lg bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10">
          <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ data.performance.avgTokenConsumption }}</p>
          <p class="text-xs text-gray-500 mt-1">平均 Token 消耗</p>
          <p class="text-[10px] text-gray-400 mt-1">/次请求</p>
        </div>
        <div class="text-center p-4 rounded-lg bg-gradient-to-br from-purple-50 to-violet-50 dark:from-purple-900/10 dark:to-violet-900/10">
          <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ data.performance.peakConcurrency }}</p>
