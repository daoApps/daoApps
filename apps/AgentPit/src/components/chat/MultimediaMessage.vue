...
<template>
  ...
  <div v-if="isImage" class="my-2">
    ...
    >
      ...
      >
        <span
          class="text-white text-sm font-medium bg-black/50 px-3 py-1.5 rounded-lg backdrop-blur-sm"
        >
          点击查看大图
        </span>
      </div>
  ...
  <div
    v-if="isFile"
    class="my-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 transition-colors cursor-pointer group"
  >
    <div class="flex items-center gap-3">
      <div
        class="w-10 h-10 flex-shrink-0 rounded-lg bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xl"
      >
      ...
      <div class="flex-1 min-w-0">
        ...
        <p class="text-xs text-gray-500 mt-0.5">
          ...
          {{ message.fileMeta!.type.split('/')[1]?.toUpperCase() || message.fileMeta!.type }}
        </p>
      </div>
      <svg
  ...
  <div
    v-if="isCode"
    class="my-2 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700"
  >
    <!-- 代码头部 -->
    <div
    ...
    <!-- 代码内容 -->
    <pre
      class="p-4 overflow-x-auto bg-gray-900 text-gray-100 text-sm leading-relaxed"
    ><code>{{ message.codeMeta!.code }}</code></pre>
  </div>
</template>
