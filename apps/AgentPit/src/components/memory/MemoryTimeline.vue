...
const eventTypeConfig: Record<TimelineEvent['type'], { icon: string; color: string; label: string }> = {
...
const filteredEvents = computed(() => {
  let events = [...props.events].sort((a, b) =>
...
<template>
  <div class="memory-timeline__container bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
    <!-- 工具栏 -->
    ...
    <div class="px-4 py-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
      <div class="text-sm text-gray-600 dark:text-gray-400">
        共 <span class="font-semibold text-gray-900 dark:text-white">{{ filteredEvents.length }}</span> 条记录
      </div>
    ...
    <div ref="timelineRef" class="relative max-h-[600px] overflow-y-auto p-6">
      <!-- 空状态 -->
      <div v-if="filteredEvents.length === 0" class="flex flex-col items-center justify-center py-16 text-gray-400">
      ...
      <div v-else class="relative">
        <!-- 中轴线 -->
        <div class="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-blue-400 via-purple-400 to-pink-400"></div>
        ...
        <TransitionGroup name="timeline" tag="div" class="space-y-6">
          ...
          >
            ...
            <div
              ...
              :style="{ backgroundColor: eventTypeConfig[event.type].color + '20', border: `3px solid ${eventTypeConfig[event.type].color}` }"
            ...
            >
                ...
                <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-gray-600">
                  <div class="flex items-center gap-2">
                    <span
                      ...
                      :style="{ backgroundColor: eventTypeConfig[event.type].color + '20', color: eventTypeConfig[event.type].color }"
                    >
              ...
              <Transition name="expand">
                <div v-if="expandedEvents.has(event.id)" class="px-4 pb-4 pt-0 bg-gray-50 dark:bg-gray-750 border-t border-gray-100 dark:border-gray-600">
                    ...
                    <div v-if="event.metadata">
                      ...
                      <div class="grid grid-cols-2 gap-2 text-xs">
                        <div v-for="(value, key) in event.metadata" :key="key" class="flex gap-2">
                          ...
                          <span class="text-gray-900 dark:text-white font-medium">{{ String(value) }}</span>
                        </div>
                      </div>
                    </div>
