...
<template>
  <div
    class="file-manager__container bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden"
  >
    <!-- 工具栏 -->
    <div
      class="file-manager__toolbar flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700"
    >
      <div class="flex items-center gap-2">
        ...
        <label
          class="px-3 py-2 text-sm font-medium text-white bg-green-500 hover:bg-green-600 rounded-lg transition-colors cursor-pointer"
        >
          📤 上传文件
          <input
            type="file"
      ...
      <div class="text-sm text-gray-500 dark:text-gray-400">共 {{ currentFiles.length }} 项</div>
    </div>

    <!-- 面包屑导航 -->
    <div
      class="file-manager__breadcrumb flex items-center gap-2 px-4 py-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700"
    >
    ...
    >
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <TransitionGroup name="list">
          ...
          <div
            v-if="isNewFolder"
            key="new-folder"
            class="file-manager__item flex flex-col items-center p-3 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-400"
          >
          ...
          <div
            ...
            :class="{
              'ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-900/20': selectedFile?.id === file.id
            }"
          ...
          >
            ...
            <p
              class="text-xs text-center text-gray-700 dark:text-gray-300 truncate w-full px-1"
              :title="file.name"
            >
              {{ file.name }}
            </p>
      ...
      <!-- 空状态 -->
      <div
        v-if="currentFiles.length === 0 && !isDragging && !isNewFolder"
