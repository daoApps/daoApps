...
<template>
  <div
    v-if="meeting"
    class="h-[calc(100vh-120px)] flex flex-col bg-gray-900 rounded-xl overflow-hidden shadow-2xl"
  >
    <div class="bg-gray-800 px-6 py-4 flex items-center justify-between border-b border-gray-700">
      ...
      <div class="flex items-center space-x-4 text-sm text-gray-300">
        ...
        >
          ...
          <span
            v-if="!showChat"
            class="absolute -top-1 -right-1 w-5 h-5 bg-blue-500 rounded-full text-xs flex items-center justify-center"
            >3</span
          >
        </button>
      </div>
    ...
    <div class="flex-1 flex overflow-hidden">
      <div class="flex-1 p-4 grid gap-4" :class="showChat ? 'grid-cols-2' : 'grid-cols-3'">
        ...
        >
          ...
          <div
            v-if="participant.isSpeaking && participant.isVideoOn"
            class="absolute bottom-0 left-0 right-0 h-1 bg-green-500"
          ></div>

          <div
            class="absolute top-3 left-3 px-3 py-1 bg-black/50 backdrop-blur-sm rounded-full text-white text-sm font-medium flex items-center space-x-2"
          >
        ...
        <div
          v-if="meeting.participants.length < 4 || showChat"
          class="bg-gray-800 rounded-lg border-2 border-dashed border-gray-600 flex items-center justify-center hover:border-gray-500 transition-colors cursor-pointer group"
        >
    ...
    <div class="bg-gray-800 px-6 py-4 border-t border-gray-700">
      <div class="flex items-center justify-center space-x-4">
        ...
        <button
          class="p-4 rounded-full transition-all duration-200"
          :class="
            !meeting.isVideoOn ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-700 hover:bg-gray-600'
          "
          @click="toggleVideo"
        >
          <svg
            v-if="meeting.isVideoOn"
            class="w-6 h-6 text-white"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
