...
const grid = ref<number[][]>(Array.from({ length: SIZE }, () => Array.from({ length: SIZE }).fill(0)) as number[][])
const score = ref(0)
...
const tileTextColors: Record<number, string> = {
  0: 'transparent', 2: '#776e65', 4: '#776e65', 8: '#f9f6f2',
  ...
  <div class="game-2048 flex flex-col items-center">
    ...
    <div id="game2048-container" class="relative bg-gray-700 p-2 rounded-xl shadow-lg select-none"
      ...
      <TransitionGroup name="tile" tag="div" class="grid gap-1.5 w-full h-full"
        ...
        <div v-for="(cell, idx) in grid.flat()" :key="`${idx}-${cell}`"
          ...
          :style="{
            ...
            boxShadow: cell >= 2048 ? '0 0 20px rgba(237, 194, 46, 0.6), inset 0 0 10px rgba(255,255,255,0.2)' : '',
          }">
          {{ cell || '' }}
        </div>
      </TransitionGroup>
      ...
      <Transition name="win-fade">
        <div v-if="showWinAnimation && !gameOver" class="absolute inset-0 bg-yellow-400/90 backdrop-blur-sm rounded-xl flex items-center justify-center z-10">
      ...
      <!-- 游戏结束遮罩 -->
      <Transition name="win-fade">
        <div v-if="gameOver" class="absolute inset-0 bg-black/60 backdrop-blur-sm rounded-xl flex items-center justify-center z-10">
          <div class="text-center p-6">
...
<style scoped>
.tile-enter-active { transition: all 0.15s ease-out; }
.tile-enter-from { transform: scale(0); opacity: 0; }
.tile-move { transition: transform 0.15s ease; }

.win-fade-enter-active, .win-fade-leave-active { transition: opacity 0.4s ease; }
.win-fade-enter-from, .win-fade-to { opacity: 0; }
