...
function draw() {
  ...
  snake.value.forEach((segment, index) => {
    ...
    if (isHead) {
      ...
      ctx.beginPath();
      ctx.arc(
        segment.x * CELL_SIZE + CELL_SIZE / 2 - 3 + eyeOffset,
        segment.y * CELL_SIZE + CELL_SIZE / 2 - 2,
        2,
        0,
        Math.PI * 2
      );
  ...
  ctx.beginPath();
  ctx.arc(
    food.value.x * CELL_SIZE + CELL_SIZE / 2 - 2,
    food.value.y * CELL_SIZE + CELL_SIZE / 2 - 2,
    1.5,
    0,
    Math.PI * 2
  );
...
function handleKeydown(e: KeyboardEvent) {
  ...
  if (newDir) {
    e.preventDefault();
    if (
      newDir.x !== -direction.value.x ||
      newDir.y !== -direction.value.y ||
    ...
    <div class="flex items-center justify-between w-full max-w-[360px] mb-3">
      <div class="flex gap-4 text-sm">
        <span class="text-gray-600 dark:text-gray-400"
          >分数：<strong class="text-green-500">{{ score }}</strong></span
        >
        <span class="text-gray-600 dark:text-gray-400"
          >最高分：<strong class="text-yellow-500">{{ highScore }}</strong></span
        >
      </div>
    </div>
      ...
      <Transition name="modal">
        ...
        >
          ...
          >
            ...
            <p v-if="score >= highScore && score > 0" class="text-yellow-500 font-medium mb-4">
              🎉 新纪录!
            </p>
            <p class="text-gray-500 text-sm mb-6">最高分：{{ highScore }}</p>
            <button
              class="w-full py-3 bg-green-500 text-white rounded-xl hover:bg-green-600 transition-colors font-bold text-lg"
              @click="startGame"
            >
              再来一局
            </button>
