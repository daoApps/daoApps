...
import { meetings as mockMeetings, meetingTypeColors, type Meeting } from '../../data/mockLifestyle'
...
function closeDayModal() {
  ...
  setTimeout(() => { selectedDate.value = null }, 200)
}
  ...
  <div class="meeting-calendar">
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        ...
        <span v-if="todayMeetingCount > 0" class="px-2 py-0.5 bg-red-500 text-white text-xs rounded-full">
...
:class="[
            ...
            <div class="mt-1 space-y-0.5">
              <template v-for="meeting in getMeetingsForDate(day).slice(0, 3)" :key="meeting.id">
                <div class="flex items-center gap-1 truncate">
                  <span class="w-1.5 h-1.5 rounded-sm flex-shrink-0" :style="{ backgroundColor: meetingTypeColors[meeting.type] }"></span>
                  <span class="text-xs text-gray-600 dark:text-gray-400 truncate">{{ meeting.title }}</span>
                </div>
              </template>
              <span v-if="getMeetingsForDate(day).length > 3" class="text-xs text-indigo-500 dark:text-indigo-400 font-medium">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showDayModal && selectedDate" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="closeDayModal">
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-hidden">
            ...
            <div class="p-5 space-y-3 overflow-y-auto max-h-[50vh]">
              <div v-if="getMeetingsForDate(selectedDate).length === 0" class="text-center py-8 text-gray-400">
                暂无会议安排
              </div>
...
v-for="meeting in getMeetingsForDate(selectedDate)" :key="meeting.id"
                  ...
                  <div class="flex justify-between items-start">
                    <div>
                      <h4 class="font-semibold text-gray-900 dark:text-white">{{ meeting.title }}</h4>
...
<style scoped>
.calendar-fade-enter-active, .calendar-fade-leave-active { transition: opacity 0.25s ease; }
.calendar-fade-enter-from, .calendar-fade-leave-to { opacity: 0; }
