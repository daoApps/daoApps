  ...
  return [5, 4, 3, 2, 1].map((star) => ({
    ...
    percentage:
      props.reviews.length > 0
        ? (props.reviews.filter((r) => Math.floor(r.rating) === star).length /
    ...
    >
      <div class="flex items-start gap-8">
        ...
        <div class="flex-1 space-y-2.5">
          <div
            v-for="{ star, count, percentage } in distribution"
            :key="star"
            class="flex items-center gap-3"
          >
            <span class="text-sm text-gray-600 dark:text-gray-400 w-12 flex-shrink-0"
              >{{ star }} 星</span
            >
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400 w-10 text-right">{{
              count
            }}</span>
      ...
      >
        ...
        >
          ...
          >
            ...
            有用 ({{ likeCounts[review.id] ?? (review.likes || 0) }})
          </button>
          <button
