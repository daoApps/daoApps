...
function getFlexloopPath(): string {
  return import.meta.env.VITE_FLEXLOOP_PATH || (import.meta.env.MODE === 'production' ? '/usr/local/bin/flexloop' : 'flexloop')
}
...
export function useFlexloop() {
  ...
  async function checkAvailability(): Promise<void> {
    ...
    try {
      ...
      version.value = null
      logMessage('Flexloop 工具不可用', { error: error instanceof Error ? error.message : String(error) })
    }
  }
