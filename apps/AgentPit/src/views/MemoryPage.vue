...
<template>
  <MainLayout>
    <div
      class="memory-page__wrapper min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-purple-900/20"
    >
      <!-- 入场动画容器 -->
      <Transition name="page-fade" mode="out-in">
        <div v-if="!isLoading" key="content" class="max-w-7xl mx-auto px-4 py-8 space-y-6">
          <!-- 页面标题栏 -->
          <div
            class="memory-page__header flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
          >
            <div>
              <h1
                class="text-3xl md:text-4xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent"
              >
                🧠 存储记忆系统
              </h1>
            ...
            <div class="flex items-center gap-3">
              <button
                class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg hover:from-blue-600 hover:to-cyan-600 transition-all shadow-md hover:shadow-lg"
              >
                ⚡ 快速操作
              </button>
            </div>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div
              class="memory-page__stat-card group relative overflow-hidden bg-white dark:bg-gray-800 rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700"
            >
              <div
                class="absolute top-0 right-0 w-24 h-24 bg-blue-500/10 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-500"
              ></div>
            ...
            >
              <div
                class="absolute top-0 right-0 w-24 h-24 bg-purple-500/10 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-500"
              ></div>
              <div class="relative z-10">
            ...
            >
              <div
                class="absolute top-0 right-0 w-24 h-24 bg-green-500/10 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-500"
              ></div>
              <div class="relative z-10">
                <div class="text-3xl mb-2">🔍</div>
                <div class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">
            ...
            >
              <div
                class="absolute top-0 right-0 w-24 h-24 bg-orange-500/10 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-500"
              ></div>
              <div class="relative z-10">
                <div class="text-3xl mb-2">💿</div>
                <div class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">
          ...
          >
            <!-- Tab 标签栏 -->
            <div
              class="memory-page__tabs flex flex-wrap gap-1 p-2 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700"
            >
              <button
                v-for="tab in tabs"
                :key="tab.id"
            ...
            <div class="memory-page__content p-4 md:p-6">
              <KeepAlive :max="6">
                <Transition name="tab-fade" mode="out-in">
                  <!-- 文件管理器 -->
                  <FileManager v-if="activeTab === 'files'" key="files" :files="mockFileTree" />
        ...
        <div v-else key="loading" class="flex items-center justify-center min-h-[600px]">
          <div class="text-center space-y-4">
            <div
              class="w-16 h-16 mx-auto border-4 border-blue-200 dark:border-blue-800 border-t-blue-500 rounded-full animate-spin"
            ></div>
